...
class Agent:
    """Single Agent with basic tool support."""
    ...
    def _estimate_tokens_fallback(self) -> int:
        ...
        # Rough estimation: average 2.5 characters per token
    ...
    async def _summarize_messages(self):
        ...
        print(
            f"{Colors.BRIGHT_YELLOW}Token estimate: {estimated_tokens}/{self.token_limit}{Colors.RESET}"
        )
        print(
            f"{Colors.BRIGHT_YELLOW}Triggering message history summarization...{Colors.RESET}"
        )

        # Find all user message indices (except system)
        user_indices = [
        ...
        if len(user_indices) == 0:
            print(
                f"{Colors.BRIGHT_YELLOW}⚠ Insufficient messages, cannot summarize.{Colors.RESET}"
            )
            return

        # Build new message list
        new_messages = [self.messages[0]]  # Start with system message
        summary_count = 0

        for i, user_index in enumerate(user_indices):
            # Add current user message
            new_messages.append(self.messages[user_index])

            # Determine the range to summarize
            # If last user, go to end of message list; otherwise to before next user
            if i < len(user_indices) - 1:
                next_user_index = user_indices[i + 1]
            else:
                next_user_index = len(self.messages)

            # Extract execution messages for this round
            execution_messages = self.messages[user_index + 1 : next_user_index]
        ...
        print(
            f"{Colors.BRIGHT_GREEN}Summary complete. tokens reduced from {estimated_tokens} to {new_tokens}{Colors.RESET}"
        )
        print(
